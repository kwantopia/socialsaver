...
class OTNUserCreationForm(forms.Form):
    ...
    """    
    mit_id = forms.RegexField(label=_("mit_id"), max_length=10, regex=r'^\d+$', required=False,
    ...
    pin = forms.RegexField(label=_("pin"), max_length=6, regex=r'^\d\d\d\d\d?\d?$', required=True,
        help_text = _("Enter 4-6 numbers."),
        error_message = _("This value must contain only 4-6 numbers."))

class HumanSubjectForm(forms.ModelForm):
    """
