...
import hashlib
from common.models import OTNUser, Experiment, OTNConsent, SharingProfile
...
def setup(request,redirect_url=None,
    ...
    if request.method == "POST":
        ...
        if request.POST.get('facebook_only',False):
            ...
            user, created = OTNUser.objects.get_or_create(username=request.facebook.uid)
            ...
            try:
                send_mail('New OTN User', msg, 'kwan@media.mit.edu', ['mitcard@mit.edu', 'kwan@media.mit.edu'], fail_silently=False) 
