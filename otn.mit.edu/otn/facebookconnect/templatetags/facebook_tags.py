...
def show_facebook_name(context,user):
    ...
    if getattr(settings,'WIDGET_MODE',None):
        #if we're rendering widgets, link direct to facebook
        return {'string':u'<fb:name uid="%s" useyou="false"  />' % (p.facebook_id)}
    else:
        return {'string':u'<a href="%s">%s</a>' % (p.get_absolute_url(),p.full_name)}

@register.inclusion_tag('facebook/show_string.html',takes_context=True)
def show_facebook_id(context,user):
    if isinstance(user,FacebookProfile):
        p = user
    else:
        p = user.facebook_profile
    if getattr(settings,'WIDGET_MODE',None):
        #if we're rendering widgets, link direct to facebook
        return {'string':u'<span>%s</span>' % (p.facebook_id)}
    else:
        return {'string':u'<span>%s</span>' % (p.facebook_id)}
...
def show_facebook_photo_small(context,user):
    if isinstance(user,FacebookProfile):
        p = user
    else:
        p = user.facebook_profile
    if getattr(settings,'WIDGET_MODE',None):
        #if we're rendering widgets, link direct to facebook
        return {'string':u'<fb:profile_pic uid="%s" facebook-logo="true" style="height:25px" />' % (p.facebook_id)}
    else:
        return {'string':u'<img src="%s" alt="%s" style="height:25px"/>' % (p.picture_url, p.full_name)}
