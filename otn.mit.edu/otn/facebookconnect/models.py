...
class FacebookProfile(models.Model):
    ...
    FACEBOOK_FIELDS = ['uid,profile_url,name,first_name,last_name,pic,pic_square,pic_big,pic_square_with_logo,affiliations,status,proxied_email']
    DUMMY_FACEBOOK_INFO = {
        'uid': 0,
        'name': '(Private)',
        'first_name': '(Private)',
    ...
    def __get_picture_large_url(self):
        self.__configure_me()
        if self.__facebook_info is None:
            return self.DUMMY_FACEBOOK_INFO['pic_big']
        elif self.__facebook_info['pic_big']:
    ...
    def __get_status(self):
        ...
        if self.__facebook_info['status']:
            return self.__facebook_info['status']['messsage']
        else:
            return ""
    ...
    def get_friends_profiles(self,limit=settings.NUM_FRIEND_RETRIEVE_LIMIT):
        ...
        if len(friends_ids) > 0:
            ...
            if len(friends_ids) > limit:
                friends, my_info = self.__get_facebook_info(friends_ids[0:limit])
            else:
                friends, my_info = self.__get_facebook_info(friends_ids)
    ...
    def __get_facebook_info(self,fbids):
        ...
        if len(ids_to_get) > 0:
            log.debug("__get_facebook_info: Calling for %s" % ids_to_get)
            tmp_info = _facebook_obj.users.getInfo(
                            ids_to_get, 
                            self.FACEBOOK_FIELDS
                        )
            log.debug("Successfully obtained facebook profile")
