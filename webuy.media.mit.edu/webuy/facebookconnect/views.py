...
from bestbuy.models import Party, Email
...
def setup(request,redirect_url=None,
    ...
    if request.method == "POST":
        ...
        if request.POST.get('facebook_only',False):
            ...
            user, created = Party.objects.get_or_create(username=request.POST['email'])
            ...
            user.image = profile.picture_url
            user.reward_zone = request.POST.get('rewardzone','000000000')
            user.otn_consent = request.POST['otnconsent']

            # assign user to experiment
            num_exp = Experiment.objects.all().count()
            if num_exp > 0:
                #exp_no = (user.id%num_exp)+1
            ...
            email = Email(email_id=request.facebook.uid, party=user,
                    type_code=Email.OTN, email=request.POST['email'])
            email.save()
