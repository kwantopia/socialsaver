...
$message = $_GET['message'] or $message = $argv[2] or $message = 'Message received from javacom';
$badge = (int)$_GET['badge'] or $badge = (int)$argv[3];
$sound = $_GET['sound'] or $sound = $argv[4];
...
if ($argv[1] == 1) {
    $body['msg_type'] = 'review_request';
    $body['item_id'] = '3';
} elseif ($argv[1] == 2) {
    $body['msg_type'] = 'review_response';
    $body['item_id'] = '5';
} elseif ($argv[1] == 3) {
    $body['msg_type'] = 'feed_alert';
    $body['feed_id'] = '5';
} else {
...
if ($production) {
    // Production
    stream_context_set_option($ctx, 'ssl', 'local_cert', '../keys/webuyprodcert.pem');
    ...
    // Sandbox
    stream_context_set_option($ctx, 'ssl', 'local_cert', '../keys/webuydevcert.pem');
    //stream_context_set_option($ctx, 'ssl', 'passphrase', $pass);
    // Sandbox
